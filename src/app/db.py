...
def save_prediction(request_id:str, feature: dict, score: float, latency_ms: float, prediction: bool,model_version: str,status_code: int):
    ...
    with psycopg.connect(settings.database_url) as conn:
        conn.execute(
            "INSERT INTO predictions (request_id,model_version,features,score,prediction,latency_ms,status_code)"
            "VALUES (%s,%s,%s,%s,%s,%s,%s)",
            (request_id,model_version,Json(feature),score,prediction,latency_ms,status_code)
        )